--[[
	AGGRESSIVE SnakeCollisionHandler with Enhanced Direct Communication (FIXED)
...
local function aggressiveWorkspaceCleanup(centerPosition, radius)
	...
	task.spawn(function()
		...
		for _, part in ipairs(allParts) do
			...
			-- Effect detection criteria (NO NEON CHECK)
			if part:IsA("BasePart") then
				...
				-- Material criteria - ForceField and Glass only (NOT Neon)
				...
				-- Generic "Part" name with small size
...
local function performGlobalCleanup()
	...
	for _, obj in ipairs(workspace:GetDescendants()) do
		if obj:IsA("BasePart") then
			...
			-- Skip neon parts (death orbs are fine)
